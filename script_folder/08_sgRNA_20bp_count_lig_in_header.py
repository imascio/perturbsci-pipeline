...
def gRNA_cell_UMI_ident(sample, input_folder, RT_barcode_list, inner_i7_bc_list,\
    ...
    ## making reference of direct capture perturbseq region to filter out bad R1 reads
    ## HOWEVER - our sequencing structure does not read into this sequence, R1 for us only has CB/UMI
    ...
    # making reference of U6 promoter to filter out bad R2 reads - should contain inner i7, then U6, then sgRNA
    ...
    while (line1):
        ...
        ### >>> CHANGED: Extract ligation barcode from header
        line1_header_str = line1_header.decode()
        lig_match = re.search(r":r([ACGT]+)", line1_header_str)
        if lig_match:
            tmp_lig = lig_match.group(1)
        else:
            # If barcode not found, skip this record
            line1 = f1.readline()
            line1 = f1.readline()
            line1 = f1.readline()
            line2 = f2.readline()
        ...
        ## constant_R1 is the location of the direct capture sequence in R1 - which we DO NOT have - commenting out
        #constant_R1 = line1[18:28].decode()

	# constant_R2 is the location of the U6 sequence in R2 - which we have
        constant_R2 = line2[10:20].decode()
        
        #use constant regions to filter pattern-matched reads and match i5 ligation barcodes
        #replaced this line "if tmp_lig in ligation_barcode_list and constant_R1 in constant_mismatch_list1 \ and constant_R2 in constant_mismatch_list2:"
        # to remove the constant_R1 stuff
        if tmp_lig in ligation_barcode_list and constant_R2 in constant_mismatch_list2:
            ...
            if inner_i7 in inner_i7_bc_list:
                ...
                # our sequencing structure was one short to include all 20 bp of sgRNA, so changing line2[35:55] to line2[35:54]
                # if you have all 20bp and 55 bases for read 2 then you use 55 instead of 54
